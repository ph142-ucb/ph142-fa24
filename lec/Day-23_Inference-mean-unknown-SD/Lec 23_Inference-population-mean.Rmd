...
- Today, we generalize the to the more practical situation where 
...
equal to $\sigma/\sqrt{n}$ and used this in our calculations
...
$\bar{x}$. $s/\sqrt{n}$ estimates how much sample *means* vary if we were to take
many multiple samples
...
- The t-test statistic is more variable, in samll sample sizes, than the z-test statistic because we have to 
estimate $\sigma$ using $s$. Because $s$ is a statistic, it varies across samples 
(whereas $\sigma$ is a population parameter, which implies it is a constant).
- This substitution makes the t-test, in small samples, not follow Normal(0, 1) distribution,
*even if the original distribution is normal*. 
- Its distribution is *more* variable than the standard Normal. Thus, for hypothesis testing and confidence intervals, when sample size is relatively small (say $<40$) we need a distribution that is like the standard Normal but a little bit wider (i.e., more
...
under the null hypothesis. This is the p-value. We use this using the R 
function `pt(q=t, df=n-1)`
...
be ask to compare the p-value to a pre-defined significance level, $\alpha$, where
...
- The t procedures are not robust against a few outliers unless the sample size is 
sufficiently large.
