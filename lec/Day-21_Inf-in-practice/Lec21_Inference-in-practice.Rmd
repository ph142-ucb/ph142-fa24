...
- We call $\bar{x}-\mu_0$ the "magnitude of the observed effect" since it is our
measure of how much the population average that we're looking at differs from some 
null hypothesized mean

- We also call $\sigma/\sqrt{n}$ the "size of chance variation" because it 
...
sources is fair game for quizzes and tests!
...
Body temperature has a known standard deviation: $\sigma=0.6 ^{\circ}F$. We want to estimate the mean
...
to the nearest whole digit (553) then the margin of error would be smaller than 
$\pm{0.05}$
...
of a size that we think is important? I.e., what is the probability of rejecting
...
$H_0$ when the true underlying mean equals $\mu_a=0.8$. That is, you want 90% 
**power** when $\mu_a=0.8$.
* What sample size will give you 90% power?

### Example of calculating power

Begin by assuming $n=10$ and calculate the minimum quantile required to reject $H_0$:

```{r}
qnorm(p = 0.05, mean = 0, sd = 1/sqrt(10), lower.tail = F)
```

So for any quantile with this value or higher, you will reject $H_0$ in favor of $H_a$.

Now suppose that $\mu_a=0.8$ is true. The test will reject $H_0$ what 
...
Thus, you have a 81.2% chance of obtaining evidence in favor of $H_a$ when $\mu=0.8$
...
ggplot(data = data.frame(x = c(-1.3, 2)), aes(x)) + 
  ...
  annotate('text', x = 0, y = 1.2, label = "Ho", check_overlap = T) +
  annotate('text', x = 0.8, y = 1.2, label = "Ha", check_overlap = T)
...
where an $\bar{x}\geq0.5201484$ would provide evidence against $H_0$ at 
$\alpha = 0.05$ .
* Then, we sketch the distribution of the alternative if $\mu_a=0.8$ and we calculate
the probability of rejecting $H_0$ when $\mu_a=0.8$ is true
...
- The difference between the null and true values being the same, smaller p-values occur with larger $n$
...
- The power is the chance of making the right decision when the alternative is true.
    - Thus, its complement, $1-\text{Power}$, is the chance of making a wrong decision 
