---
title: "Lecture 20: Hypothesis Tests for a Mean with a Known Standard Deviation"
subtitle: Chapter 14
...
standard deviation (SD) is known
...
* [Chapter 14 of Baldi and Moore](https://ebookcentral-proquest-com.libproxy.berkeley.edu/lib/berkeley-ebooks/reader.action?docID=6643934&ppg=660)

### Last lecture

- We covered how to make a confidence interval (CI) around the sample mean and how to
...
following a Normal distribution with the mean $\mu = 1.2$ mmol/l (millimoles per liter) and standard deviation $\sigma = 0.1$ mmol/l.

A study examined inorganic phosphorus in older individuals **to see if its lower among
older adults.** Here are the data from 12 people between the ages of 75 and 79:
...
* So the question is: Is there evidence that levels of inorganic phosphorus are lower
among older adults?
...
Often, one uses the mean level to investigate the distribution, so in this case we proposed the mean level of inorganic phoshporous as the parameter of interest.

**Step 2: Specify the null hypothesis, $H_0$**

* The null hypothesis assumes that the data is sampled from a distribution for 
which the underlying mean is indeed equal to 1.2. We will look to see how much 
evidence there is against the null hypothesis (conversely, how much evidence in favor of the alternative hypothesis).
...
* Re-read the question. We want to know if the average for elderly patients is lower 
than the overall average. This dictates the direction of the alternative hypothesis.
...
the one we got *if the data are distributed according to the null distribution* -- is called the **p-value**.
...
### Step 4: Significance level $\alpha$ 
* The $\alpha$ level is the desired type I error rate.  This is the tolerable probability of false positives (rejection of the null when it is true) in the theoretical context of repeated experiments (optional).
...
* In class, we will often ask you to calculate the probability, but not always
to compare it to 5%. This is because we want you to become familiar with interpreting
...
mean at least as small as what we saw

- This is a very tiny probability, and 
...
has a known standard deviation of 5 mg. Furthermore, you have an SRS of $n=10$ pills
...
* The only difference is how the probability is calculated. Here, we are interested
...
arbitrary cutpoint. It is much better to report the exact value of the p-value than
...
informative to report the p-value you calculated than to only make a statement 
...
$\bar{x}=1.19$ is statistically different from $\mu=1.2$ (say at $\alpha=0.05$),
...
in practice is not determined by the data, but based on judgement of decision-makers:

  - What is a meaningful reduction in depressive symptoms associated with a new,
...
The Z test for a population mean: Draw an SRS from a Normal($\mu$, $\sigma$) 
...
p-value is represented below for upper tail one-sided, lower tail one-sided, and two-sided tests: 
...
* The CI is better because it puts a range around the value of 
the parameter
