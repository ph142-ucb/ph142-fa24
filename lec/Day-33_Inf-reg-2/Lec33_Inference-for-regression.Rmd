...
subtitle: "Chapter 23 (continued)"
author: "Corinne Riddell (Instructors: Alan Hubbard & Tomer Altman)"
...
```{r frog-data, echo = F, fig.align='center', out.width="50%"}
...
the fitted y-values ($\hat{y}$) and the residuals ($\hat{e} = y-\hat{y}$) among other columns (you don't need to worry about the other columns that are added)
...
- $sigma = 2.82$. This is the residual estimated standard deviation (or sometimes called residual standard error).
...
ggplot(frog_data, aes(x = temp, y = freq)) +
  ...
  geom_text(aes(x = 20, y = 48), label = "Null hypothesized slope", col = "red", check_overlap = T) +
...
Interpretation: The estimate for the slope coefficient is 2.33 (95% CI: 1.60 to 3.06). If we had gather 100 random samples and ran the same regress on each of them and used the same method to compute the 95% CI, 95 out of the 100  intervals would contain the true value of $\beta$ in the interval. 
